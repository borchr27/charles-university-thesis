...
where \(\Gamma\) is the gamma function and \(x\) is a random vector.
\end{defn}

\begin{defn}[Expected Error Reduction]
\label{def:expected_error_reduction}
Expected error reduction is a framework that uses the expected error of the classifier after labeling the candidate minus the expected error of the classifier before labeling the candidate to determine which candidate to label. The expected error of the classifier is the expected value of the error measure given the posterior distribution of the classifier.
\end{defn}
...
The posterior probability is a type of conditional probability that results from updating the prior probability with information summarized by the likelihood via an application of Bayes' rule. The posterior probability is the probability of an event occurring given that another event has occurred.
\end{defn}

\begin{defn}[Omniscient Oracles]
\label{def:omniscient_oracles}
Omniscient oracles are oracles that have access to the ground truth.
\end{defn}
